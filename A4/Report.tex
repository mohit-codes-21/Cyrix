...
\noindent As an optional extension (for curiosity and verification), the same IR is lowered to x86-64 machine code in WSL and executed to print final variable values. This does not change the language or IR; it is a post-pass.
...
\texttt{int}, \texttt{float}, \texttt{void}.

\noindent \texttt{void} variables can be declared but cannot be assigned or used in expressions or conditions. In the optional machine-code backend, \texttt{float} values are represented as \textbf{double-precision} for simpler runtime printing.
...
\subsection{Type Checking and Implicit Casting}

The compiler performs static type checking and inserts explicit cast quads when needed. Arithmetic and relational operators promote operands to a common numeric type (\texttt{float} dominates \texttt{int}). Assignments cast the RHS to the LHS type. The cast operations are emitted as quads and shown in the \texttt{expr} column as function-like calls.

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Cast} & \textbf{Quad Emitted} & \textbf{Expr Column} \\
\midrule
\texttt{int} $\rightarrow$ \texttt{float} & \texttt{(itof, x, -, t)} & \texttt{t= itof(x)} \\
\texttt{float} $\rightarrow$ \texttt{int} & \texttt{(ftoi, x, -, t)} & \texttt{t= ftoi(x)} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
    ...
    \item \texttt{int x = E;} $\rightarrow$ symbol table entry \textbf{plus} \texttt{(=, E, -, x)} (with implicit cast if needed).
    \item \texttt{void x;} is allowed, but any initialization or assignment to \texttt{x} is a semantic error.
\end{itemize}
...
The symbol table is printed after every statement, showing scope depth, declaration line, live-until (either ``active'' or the closing line number), initialisation status, and the last known value. The value \texttt{?} means ``unknown'' (used when a value is control-flow dependent or not yet assigned).
...
Void variable initialisation / assignment & \texttt{declaration\_statement}, \texttt{assignment\_expression} & \texttt{[Semantic Error] Line N: void variable 'x' cannot be initialized} \\
Void value used in expression or condition & \texttt{primary\_expression}, condition rules & \texttt{[Semantic Error] Line N: void value used in expression} \\
...
\begin{itemize}
    ...
    \item \textbf{Conservative unknowns inside control flow}: assignments inside \texttt{if/else} blocks or loops mark the value as \texttt{?} because the assignment may or may not execute along all paths. This keeps the symbol table consistent without changing the IR.
\end{itemize}

% ════════════════════════════════════════════════════════════════════════
% ════════════════════════════════════════════════════════════════════════
\section{Part 5: Optional Machine Code Generation (WSL)}
% ════════════════════════════════════════════════════════════════════════

This part is not required by the assignment; it was added for curiosity and to validate the IR by executing real code. The backend is a post-pass over the quad table and does not change the grammar or SDT actions.

\subsection{Target and Toolchain}

The backend emits x86-64 (AMD64) assembly for the System V ABI and is built and run in WSL using GCC. This path is intentionally limited to WSL; native Windows shells are not supported.

\subsection{Lowering Rules (Summary)}

\begin{itemize}
    \item Arithmetic: \texttt{+,-,*,/} map to \texttt{addsd/subsd/mulsd/divsd} for float and \texttt{addl/subl/imull/idivl} for int.
    \item Comparisons: integer compares use \texttt{cmpl + setcc}; float compares use \texttt{ucomisd + setcc}.
    \item Control flow: \texttt{ifFalse} and \texttt{goto} become conditional and unconditional jumps, and \texttt{label} emits assembly labels.
    \item Casts: \texttt{itof} uses \texttt{cvtsi2sd}, and \texttt{ftoi} uses \texttt{cvttsd2si}.
\end{itemize}

\subsection{Float Handling}

Although the source language uses \texttt{float}, the backend stores and computes all float values as \textbf{double-precision}. This simplifies \texttt{printf} calls in a varargs ABI and avoids mixed-size conversions.

\subsection{Runtime Printing Policy}

Instead of adding a new print statement to the language, the backend prints a final variable dump after program completion and appends it to the output file. A banner separates the runtime dump from the IR output. Uninitialized variables are printed with a tag, and memory is zero-initialized to keep the dump deterministic.

\subsection{Scoping Bug and Fix}

The IR stores variables by name only. In the first backend version, a name always mapped to the most recently allocated slot, which broke shadowing in nested scopes (e.g., test 08). The fix resolves a variable by checking its declaration line and scope end against the quad's source line, selecting the innermost symbol active at that line. This preserves correct scoping without modifying the IR.

\subsection{Error Handling in Codegen}

If any parse or semantic error occurs, the compiler suppresses IR output and does not emit assembly or executables. This prevents running incomplete or inconsistent code.
...
\subsubsection{Test 01 --- Basic Arithmetic + Casting (Excerpt)}
...
|    2 | b=10        | =     | 10   | -    | b    | line 3  |
|    3 | t1=a+b      | +     | a    | b    | t1   | line 4  |
|    4 | t2= itof(t1)| itof  | t1   | -    | t2   | line 4  |
|    5 | c=t2        | =     | t2   | -    | c    | line 4  |
|    6 | t3=a*b      | *     | a    | b    | t3   | line 5  |
|    7 | t4= itof(t3)| itof  | t3   | -    | t4   | line 5  |
|    8 | t5=t4-c     | -     | t4   | c    | t5   | line 5  |
|    9 | t6= ftoi(t5)| ftoi  | t5   | -    | t6   | line 5  |
|   10 | d=t6        | =     | t6   | -    | d    | line 5  |
|   11 | t7=d/2      | /     | d    | 2    | t7   | line 7  |
|   12 | e=t7        | =     | t7   | -    | e    | line 7  |
\end{lstlisting}

\textbf{Final variable dump (runtime):}
\begin{lstlisting}[style=outputstyle]
a = 5
b = 10
c = 15.000000
...
The error-handling tests explicitly validate that the compiler detects semantic/lex/parse errors, buffers them, and suppresses IR output. The table below summarizes the intent, followed by the exact sources and the error messages observed in the output files.
...
\midrule
11 & Redeclaration in same scope + void init & 2 semantic errors & \texttt{[FAIL]}, IR suppressed \\
12 & Use of undeclared variables & 2 semantic errors & \texttt{[FAIL]}, IR suppressed \\
13 & \texttt{break}/\texttt{continue} outside loop & 2 semantic errors & \texttt{[FAIL]}, IR suppressed \\
14 & Unknown character in expression & 1 lex + 1 parse error & \texttt{[FAIL]}, IR suppressed \\
15 & Mixed: inner redeclaration + undeclared var & 2 semantic errors & \texttt{[FAIL]}, IR suppressed \\
\bottomrule
\end{tabular}
\end{center}

\subsubsection{Test 11 --- Redeclaration + void initialization}
...
\textbf{Errors reported:}
\begin{lstlisting}[style=outputstyle]
[Semantic Error] Line 4: 'x' already declared in this scope (first declared at line 2)
[Semantic Error] Line 5: void variable 'z' cannot be initialized
\end{lstlisting}

\subsubsection{Test 12 --- Use of undeclared variables}
...
\textbf{Errors reported:}
\begin{lstlisting}[style=outputstyle]
[Semantic Error] Line 3: use of undeclared variable 'c'
[Semantic Error] Line 5: use of undeclared variable 'e'
\end{lstlisting}

\subsubsection{Test 13 --- Control jumps outside loop}
...
\textbf{Errors reported:}
\begin{lstlisting}[style=outputstyle]
[Semantic Error] Line 3: 'break' outside a loop
[Semantic Error] Line 5: 'continue' outside a loop
\end{lstlisting}

\subsubsection{Test 14 --- Unknown characters (lex + parse)}
...
\textbf{Errors reported:}
\begin{lstlisting}[style=outputstyle]
[Lex Error]    Line 3: unknown character '@'
[Parse Error]  Line 3 near '3': syntax error, unexpected I_CONSTANT, expecting ';'
\end{lstlisting}

\subsubsection{Test 15 --- Mixed: redeclaration + undeclared variable}
...
\textbf{Errors reported:}
\begin{lstlisting}[style=outputstyle]
[Semantic Error] Line 6: 'a' already declared in this scope (first declared at line 5)
[Semantic Error] Line 7: use of undeclared variable 'q'
\end{lstlisting}

Because errors suppress IR, the machine-code backend is also skipped and no assembly or executable is produced for these tests.
...
make tests-asm                # generate .s + .exe for all valid tests (WSL)
make tests-run                # run .exe and append runtime output (WSL)
...
formatted IR output
$\xrightarrow{\text{codegen (WSL)}}$
\texttt{.s} $\rightarrow$ \texttt{gcc} $\rightarrow$ \texttt{.exe} $\rightarrow$ final variable dump
\end{center}
...
Mohit & \textit{SDT for basic constructs; type checking and implicit casting logic; IR output formatting; machine-code generation backend.} \\[0.5cm]
...
The Simple compiler correctly implements the four required parts of Assignment 4, plus an optional Part 5:
...
\begin{itemize}
    ...
    \item \textbf{Part 4} --- Nine distinct error categories detected and reported clearly, with IR suppressed on any error.
    \item \textbf{Part 5 (optional)} --- A WSL-only machine-code backend that executes the quads and prints final variable values for verification.
\end{itemize}

The compiler has been tested with 16 programs (10 standard + 5 erroneous + 1 PDF example) and all outputs have been verified correct. The optional machine-code path further confirms correctness by matching runtime final values with the IR semantics.
